...
**OS version confirmation** — Do not let planning proceed with "latest" or "TBD" as the target OS version. Get a written decision in Week 1. Everything downstream — hardware compatibility, application compatibility, licensing, provisioning requests — depends on it.
...
**Goal:** Document everything before touching anything.
...
**End of Phase 1 deliverable:** A gap report and risk assessment delivered to the project sponsor. Do not proceed to planning until this has been reviewed and acknowledged.
...
Two decisions in this phase are non-negotiable and must be made in writing:
...
**The 2-week next-steps presentation** (if agreed with the client) belongs in this phase. It should cover: OS upgrade sequencing, separation scope, dependency audit findings, and proposed timeline with risks called out explicitly.
...
This phase is often underestimated because the OS upgrade itself is technically straightforward. As of Server 2025, direct in-place upgrade from Server 2012 R2 is supported in a single hop and takes under an hour per server. The complexity is not the OS upgrade — it is validating that SAS, Spotfire, and Power BI Gateway all behave correctly on the new OS.
...
**Application compatibility** is the highest risk item in this phase. Check vendor support matrices before the upgrade begins, not after.
...
This is where the new IT domain comes into play. A new company domain means new AD, new DNS, new network configuration, and new service accounts. IT provisioning must be complete before you can begin this phase — which is why provisioning requests should have been submitted in Phase 0.
